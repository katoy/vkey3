...
(function($) { // hide the namespace
   ...
   allowing multiple different settings on the same page. */
    function Keypad() {
        ...
        this.addKeyDef('KANA', 'kana', function(inst) { // katoy
            if (inst.kana === undefined) {
                inst.kana = false;
            }
            inst.kana = !inst.kana;
            if (inst.kana === false ) {
                inst.settings.kanaText = '英数';
                inst.settings.kanaStatus = '';
            } else {
                inst.settings.kanaText = '仮名';
                inst.settings.kanaStatus = '';
            }
        ...
        this.addKeyDef('ZENKAKU', 'zenkaku',
            function(inst) {
                if (inst.katahira === true) {
                    if (inst.zenkaku === undefined) {
                        inst.zenkaku = false;
                    }
                    inst.zenkaku = !inst.zenkaku;
                    $.keypad._set_zenkaku_label(inst);
                    $.keypad._updateKeypad(inst);
                    inst._input.focus(); // for further typing
                }
            });

        this.addKeyDef('KATAHIRA', 'katahira',
            function(inst) {
                if (inst.katahira === undefined) {
                    inst.katahira = true;
                }
                inst.katahira = !inst.katahira;
                if (inst.katahira === false ) {
                    inst.settings.katahiraText = 'カナ';
                    inst.settings.katahiraStatus = 'カタカナ';
                } else {
                    inst.settings.katahiraText = 'ひら';
                    inst.settings.katahiraStatus = 'ひらがな';
                }
        ...
        this.addKeyDef('SHIFT2', 'shift2', function(inst) {
            $.keypad._shiftKeypad(inst);
        });
        ...
        // katoy
        this.qwertyLayout_and_kana = [
        '!@#$%^&*()_=' + this.HALF_SPACE + this.SPACE + this.HALF_SPACE + this.CLOSE,
        this.HALF_SPACE + '`~[]{}<>\\|/' + this.SPACE + this.HALF_SPACE + '789',
        'qwertyuiop\'"' + this.HALF_SPACE + this.HALF_SPACE + '456',
        this.HALF_SPACE + 'asdfghjkl;:' + this.SPACE + this.HALF_SPACE + '123',
        this.SPACE + 'zxcvbnm,.?' + this.SPACE + this.HALF_SPACE + this.HALF_SPACE + '-0+',
        '' + this.TAB + this.ENTER + this.SPACE_BAR + this.KANA + this.SHIFT2 +
        this.BACK + this.CLEAR];

        // Standard US keyboard Japanese layout
        this.kanaR = [
        "ゃぁぱばだざが" + "ゎ" + this.HALF_SPACE + "んわらやまはなたさかあ",
        "ゅぃぴびぢじぎ" + "ゕ" + this.HALF_SPACE + "、ゐり｜みひにちしきい",
        "ょぅぷぶづずぐ" + "っ" + this.HALF_SPACE + "。　るゆむふぬつすくう",
        "゛ぇぺべでぜげ" + "ゖ" + this.HALF_SPACE + "「ゑれーめへねてせけえ",
        "゜ぉぽぼどぞご" + "ゔ" + this.HALF_SPACE + "」をろよもほのとそこお",
        // "（）｛｝［］「」" + this.SPACE + "１２３４５６７８９０",
        // "、。・”’＼？！" + this.SPACE + "＄￥＋ー＝＊／．，＠",
        "" +
        this.TAB + this.ENTER + this.SPACE_BAR +
        this.KANA + this.HALF_SPACE +
        this.ZENKAKU  + this.KATAHIRA +
        this.CLEAR + this.BACK +
        // this.keypad.START + this.END +
        this.CLOSE
        ];

        this.kanaL = [
        "あかさたなはまやらわん" + this.HALF_SPACE + "ゎ" + "がざだばぱぁゃ",
        "いきすちにひみ｜りゐ、" + this.HALF_SPACE + "ゕ" + "きじぢびぴぃゅ",
        "うくすつぬむゆる　。　" + this.HALF_SPACE + "っ" + "ぐずづぶぷぅょ",
        "えけせてねへめーれゑ「" + this.HALF_SPACE + "ゖ" + "げぜでべぺぇ゛",
        "おこそちのほもろよを」" + this.HALF_SPACE + "ゔ" + "ごぞどぼぽぉ゜",
        // "（）｛｝［］「」" + this.SPACE + "１２３４５６７８９０",
        // "、。・”’＼？！" + this.SPACE + "＄￥＋ー＝＊／．，＠",
        "" +
        this.TAB + this.ENTER + this.SPACE_BAR +
        ...
        this.regional[''] = { // Default regional settings
            ...
            kanaStatus: '',
            shift2Text: 'シフト', // Display text for shift link
    ...
    $.extend(Keypad.prototype, {
	   ...
	   @return  (jQuery) the HTML for this keypad */
        _generateHTML: function(inst) {
            ...
            if (inst.kana === true) {  // katoy
                layout = this.qwertyLayout_and_kana;
            }
            ...
            for (var i = 0; i < layout.length; i++) {
                ...
                for (var j = 0; j < keys.length; j++) {
                    ...
                    if (keys[j].length === 1) {    // katoy
                        if(inst.katahira === true) {
                            keys[j] = keys[j].toKatakanaCase();
                        } else {
                            keys[j] = keys[j].toHiraganaCase();
                        }
                        if(inst.zenkaku === true) {
                            keys[j] = keys[j].toZenkakuCase().toZenkanaCase();
                        } else {
                            keys[j] = keys[j].toHankakuCase().toHankanaCase();
                        }
                    }
                    ...
                    if (keyDef) {
                        html += (keyDef.action ? '<button type="button" class="keypad-special keypad-' +
                            keyDef.name +
                            (useTR ? ' ui-corner-all ui-state-default' +
                                (keyDef.noHighlight ? '' : ' ui-state-highlight') : '') +
                            ((keyDef.name === 'zenkaku' && !inst.katahira)? ' disabled ' : '') +
	   ...
	   @param  values  (string[]) the array to rearrange */
        ...
        _set_zenkaku_label: function(inst) {
            if (inst.katahira === false ) {
                inst.settings.zenkakuText = '-';
                inst.settings.zenkakuStatus = '半角・全角の切り替え (半角平仮名はありません)';
            } else {
                if (inst.zenkaku === false ) {
                    inst.settings.zenkakuText = '全角';
                    inst.settings.zenkakuStatus = '半角・全角の切り替え';
                } else {
                    inst.settings.zenkakuText= '半角';
                    inst.settings.zenkakuStatus = '半角・全角の切り替え';
                }
            }
        }
